# -*- coding: utf-8 -*-
"""
Complete Test for Colloquial Queries - Final Report
تست کامل سوالات محاوره‌ای - گزارش نهایی
"""
...
async def test_complete():
    """تست کامل با گزارش نهایی"""
    
    print("🧪 Complete Test: Colloquial Queries - Final Report\n")
    print("="*80 + "\n")
    ...
    for i, query in enumerate(test_queries, 1):
        ...
        if normalized != query:
            ...
            print(f"   ⚠️  Normalization: Not applied")
        ...
        try:
            answer = await rag.retrieve_and_answer(
                ...
                top_k=5,
                conversation_id="test_colloquial_complete"
            )
            ...
            is_relevant = any(keyword in answer_text.lower() for keyword in 
                            ['صندوق', 'باور', 'ایمیل', 'راه', 'ارتباط', 'سرمایه', 'تمرکز'])
    ...
    normalized = sum(1 for r in results if r.get("normalized", "") != r.get("query", ""))
    ...
    print(f"🔍 Normalized: {normalized}/{len(test_queries)} ({normalized/len(test_queries)*100:.1f}%)")
    ...
    for i, result in enumerate(results, 1):
        ...
        normalized_marker = "🔍" if result.get("normalized", "") != result.get("query", "") else "  "
        relevant_marker = "📊" if result.get("is_relevant") else "  "
        
        print(f"   {status} Q{i}: {duration:6.2f}s {normalized_marker} {relevant_marker} {query_short}")
...
def main():
    ...
    try:
        return loop.run_until_complete(test_complete())
    finally:
        loop.close()
